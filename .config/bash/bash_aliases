...
alias l.t='ls -lahtc --color --time-style=iso'  # sort by mtime
alias l.u='ls -lahtu --color --time-style=iso'  # sort by atime
...
alias g.t='git log --tags --simplify-by-decoration --pretty="format:%<(16)%D - %h - %ar"'  # tags
...
alias m.d='mvn dependency:get'                  # dependency get
alias m.dt='mvn dependency:tree -Dverbose'      # dependency tree
alias m.da='mvn dependency:analyze'             # dependency analyze
alias m.do='mvn de.qaware.maven:go-offline-maven-plugin:resolve-dependencies -DdownloadSources -DdownloadJavadoc; mvn clean install -DskipTests=true'  # go offline
...
alias d.x='docker rm'
alias d.xa='docker stop $(docker ps -q); docker container prune; docker volume prune --filter "label!=keep"; docker network prune'
alias d.xA='docker system prune --volumes'
